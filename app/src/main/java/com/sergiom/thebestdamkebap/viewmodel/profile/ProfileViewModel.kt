 ...
 *
 * - No conoce Firebase (usa repos de dominio).
 * - Expone UiState + eventos efímeros.
 * - Valida con el UseCase de dominio (errores como códigos neutrales).
 */
@HiltViewModel
class ProfileViewModel @Inject constructor(
    ...
    /** Estado del formulario (valores + errores + canSave). */
    data class FormState(
        val givenName: String = "",
        val familyName: String = "",
        val phone: String = "",
        val birthDateMillis: Long? = null,
        // errores (códigos neutrales)
        val eGivenName: ProfileError? = null,
        val eFamilyName: ProfileError? = null,
        val ePhone: ProfileError? = null,
        val eBirthDate: ProfileError? = null,
        val canSave: Boolean = true
    )

    /** Estado global para Compose. */
    data class UiState(
                ...
                .collectLatest { du ->
                    when {
                        du == null || du.isAnonymous -> {
                            ...
                            // Observa perfil; si no existe → ensure
    ...
    /** Aplica el use case y vuelca los **códigos** de error al form. */
